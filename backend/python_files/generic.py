...
def has_model_perm(user, model, perm='any'):
    if not Django_Mode == 'Development1':
        return True
...
class ReactHookForm:
    ...
    __recur_field_list = ['cipher_text', 'salt', 'nonce', 'tag', 'add_by', 'modify_by',
    ...
    def get_params_response(self, *args, **kwargs):
        ...
        if get_perms:
            if Django_Mode == 'Development1':
                ...
                response['permissions'] = {'__add': True, '__change': True, '__view': True, '__delete': True}
            ...
            if k in data and data[k]:
                if isinstance(v, PrimaryKeyRelatedField):
                    if type(data[k]) == dict and 'value' in data[k]:
                        data[k] = self.decrypt_id(data[k]['value'])
                if isinstance(v, ChoiceField):
                    if type(data[k]) == dict and 'value' in data[k]:
                        data[k] = data[k]['value']
                if isinstance(v, ManyRelatedField):
                    data[k] = [self.decrypt_id(row['value']) for row in data[k] if type(row) == dict and 'value' in row]
        return data
...
class TechnoFetchAPIView(TechnoFetchMixin,
                         TechnoPermissionMixin,
                         TechnoGenericBaseAPIView):
