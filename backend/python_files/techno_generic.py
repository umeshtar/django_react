...
from rest_framework.fields import ChoiceField, DecimalField, FloatField, IntegerField
...
from app_permission.models import CustomPermission, ModuleConfiguration
    ...
    if isinstance(exc, ValidationError):
        ...
        for k, v in exc.detail.items():
            if isinstance(v, list):
                ...
                form_errors[k] = v
        return Response(
            {"form_errors": form_errors}, status=status.HTTP_400_BAD_REQUEST
        )

    if isinstance(exc, ProtectedError):
        return Response(
            {"message": "Can not delete due to protected records"},
...
class ReactHookForm:
    ...
    def get_configs(self, fields=(), exclude=()):
        ...
        if fields:
            serializer_fields = {
                k: v for k, v in serializer_fields.items() if k in fields
            }
        if exclude:
            serializer_fields = {
        ...
        for key, field in serializer_fields.items():
            if key in self.__verbose_name:
                ...
                field_name = get_field_verbose_name(
                    model=self.__serializer.Meta.model, field_name=key
                )
            configs[key] = dict()
    ...
    def __get_validators(self, field, field_type, field_name):
        ...
        if validators:
            validators.update(
                dict(name=field_name, type=field_type, stopMode=self.__stop_mode)
            )
    ...
    def __get_field_type(self, field):
        if (
            field.style
            and "base_template" in field.style
            and field.style["base_template"] == "textarea.html"
        ):
            return "textarea"
        ...
        if queryset and queryset.exists():
            ...
            for inst in queryset:
                dic = {
                    "value": inst.pk,
                    "label": (
                        self.__label[key](inst) if key in self.__label else str(inst)
                    ),
                }
                if key in self.__select_options_func:
                    dic.update(**self.__select_options_func[key](inst))
                if type(inst).__name__ == "Tbl_Country_Code" and hasattr(
                    inst, "country_code"
                ):
...
class TechnoSerializerValidation:
    ...
    def __add_error(self, field):
        ...
        error_msg = self.__error_msg[calling_func].format(
            field=self.__get_verbose_name(field)
        )
        ...
        if not field == field2:
            ...
            if value1 and value2 and value1 == value2:
                error_msg = self.__error_msg["check_same_value"].format(
                    field1=self.__get_verbose_name(field),
                    field2=self.__get_verbose_name(field2),
                )
                self.__custom_errors[field].append(error_msg)
    ...
    def check_exists(self, *args):
        for field in args:
            ...
            if value:
                filter_dic = {
                    f"{field}__iexact" if isinstance(value, str) else field: value
                }
    ...
    def check_unique_set(self, *args):
        ...
        for field in args:
            value = self.__attrs.get(field, None)
            if value:
                filter_dic[f"{field}__iexact" if isinstance(value, str) else field] = (
                    value
                )
        qs = self.__model.objects.filter(**filter_dic)
        if self.__instance is not None:
    ...
    def check_future_datetime(self, *args):
        for field in args:
            ...
            if (
                value
                and type(value).__name__ == "date"
                and value > timezone.now().date()
            ):
                self.__add_error(field)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.Meta.model or not self.Meta.fields:
            raise ImproperlyConfigured(
                f"model and/or fields Missing in {self.__class__.__name__} Meta Class"
            )
        self.tsv = TechnoSerializerValidation(
            model=self.Meta.model, instance=self.instance
        )
...
class TechnoSelectSerializer(ModelSerializer):
    class Meta:
        model = None
        fields = []
...
class TechnoPermissionMixin:
    ...
    def get_extra_modules_permissions(self):
        ...
        ).filter(codename__in=self.modules)
        check_repeat = []
        perms = defaultdict(dict)
        for config in configs:
            for perm in config.permissions.all():
                app_model_name = (
                    f"{perm.content_type.app_label}.{perm.content_type.model}"
                )
                ...
                if model_name not in perms:
                    ...
                    if app_model_name in check_repeat:
                        perms[model_name][perm_name] = self.request.user.has_perm(
                            perm_code
                        )
                    else:
                        perms[app_model_name][perm_name] = self.request.user.has_perm(
                            perm_code
                        )
        return perms
    ...
    def get_custom_permission(self):
        perms = dict()
        q_object = Q(modules__codename__in=self.modules) | Q(
            perm_scope__in=["Modules", "Global"]
        )
    ...
    def fetch(self, *args, **kwargs):
        ...
        if get_form_configs:
            response["form_configs"] = (
                self.get_form_configs() if (can_add or can_change) else dict()
            )
        ...
        if get_data:
            ...
            if is_form:
                response["data"] = (
                    ...
                    if (can_add or can_change)
                    else dict()
                )
            else:
                response["data"] = (
                    self.get_detail_serializer(self.get_object()).data
                    if can_view
                    else dict()
                )
...
class TechnoCreateMixin:
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response(
            {
                "data": (
                    self.get_list_serializer(inst).data
                    if request.user.has_model_perms(self.model, "view")
                    else dict()
                ),
...
class TechnoUpdateMixin:
    ...
    def update(self, request, *args, **kwargs):
        s = self.get_serializer(
            data=self.get_payload_data(), instance=self.get_object()
        )
        s.is_valid(raise_exception=True)
        inst = s.save()
        return Response(
