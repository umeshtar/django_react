...
class ModuleConfigurationAdmin(RecurAdmin):
    list_display = ("name", "get_children", "get_parents", "is_root_menu", "menu_type", "sequence")
    list_editable = ("is_root_menu", "sequence")
    filter_horizontal = ("permissions",)

    def get_children(self, obj):
        return ", ".join([row.name for row in obj.children.all()])

    def get_parents(self, obj):
        return ", ".join([row.name for row in obj.parents.all()])
