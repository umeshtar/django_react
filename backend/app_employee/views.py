...
class DepartmentView(TechnoGenericAPIView):
    ...
    def get_form_configs(self):
        form_configs = ReactHookForm(serializer=self.get_serializer_class()).get_configs()
        form_configs['repeaters'] = dict()
        form_configs['repeaters']['employees'] = ReactHookForm(serializer=self.employee_serializer_class).get_configs(exclude=['department'])
